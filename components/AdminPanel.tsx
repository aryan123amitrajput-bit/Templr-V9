        ...
        <div className="p-6 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/50 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={runSync}
              disabled={isSyncing}
              className="px-4 py-2 bg-blue-500 hover:bg-blue-600 disabled:opacity-50 text-white rounded-xl text-sm font-medium flex items-center gap-2 transition-all shadow-lg shadow-blue-500/20"
            >
              <Github className={`w-4 h-4 ${isSyncing ? 'animate-spin' : ''}`} />
              {isSyncing ? 'Syncing...' : 'Sync & Localize'}
            </button>
            <button 
              onClick={runAudit}
              disabled={isAuditing}
