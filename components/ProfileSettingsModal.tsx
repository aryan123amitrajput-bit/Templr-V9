...
const ProfileSettingsModal: React.FC<ProfileSettingsModalProps> = ({ isOpen, onClose, session, onShowNotification }) => {
  const [name, setName] = useState(session?.user.user_metadata?.full_name || '');
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (session) setName(session.user.user_metadata?.full_name || '');
  }, [session]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    playClickSound();
    try {
        await updateUserProfile(session.user.id, { full_name: name });
        playSuccessSound();
