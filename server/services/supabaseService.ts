...
export function getSupabase() {
  if (!supabaseClient) {
    ...
    // Prefer service role key for backend operations, fallback to anon key (either generic or VITE_ prefixed)
...
export async function uploadToSupabase(fileBuffer: Buffer, fileName: string, mimetype: string): Promise<string> {
  ...
  console.log(`[Supabase] Uploading to ${bucketName}/${filePath}...`);
